...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	if (empty($address)) {
		$lnameError = 'Please enter Address';
		$valid = false;
	}
	if (empty($city)) {
		$lnameError = 'Please enter City';
		$valid = false;
	}
	if (empty($state)) {
		$lnameError = 'Please enter State';
		$valid = false;
	}
	if (empty($zipcode)) {
		$lnameError = 'Please enter Zip Code';
		$valid = false;
	}
	...
	if ($valid) { // if valid user input update the database
		...
		if($fileSize > 0) { // if file was updated, update all fields
			...
			$sql = "UPDATE fr_persons  set fname = ?, lname = ?, address = ?, city = ?, state = ?, zipcode = ?,  email = ?, mobile = ?, password = ?, title = ?, filename = ?,filesize = ?,filetype = ?,filecontent = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($fname, $lname, $address, $city, $state, $zipcode, $email, $mobile, $password, $title, $fileName,$fileSize,$fileType,$content, $id));
			Database::disconnect();
			...
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE fr_persons  set fname = ?, lname = ?, address = ?, city = ?, state = ? , zipcode = ?,  email = ?, mobile = ?, password = ?, title = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($fname, $lname, $address, $city, $state, $zipcode, $email, $mobile, $password, $title,  $id));
			Database::disconnect();
			header("Location: fr_persons.php");
		}
	}
